...
function countVowels(str) {
  ...
  if (!str.match(regex) || str === null) return 0;
...
function isPalindrome(str) {
  ...
  const strReversed = str
    ...
    .replaceAll(regex, '')
    .toLowerCase();
  return str.toLowerCase().replaceAll(regex, '') === strReversed;
}
...
function encodeToRot13(str) {
  const array = str.split('').map((el) => {
    if (
      el !== ' ' &&
      el !== '!' &&
      el !== '?' &&
      ((el.charCodeAt(0) <= 77 && el.charCodeAt(0) >= 65) ||
        (el.charCodeAt(0) >= 97 && el.charCodeAt(0) <= 109))
    ) {
    ...
    if (
      ...
      el !== '!' &&
      el !== '?' &&
      ((el.charCodeAt(0) > 77 && el.charCodeAt(0) < 91) ||
        (el.charCodeAt(0) > 109 && el.charCodeAt(0) < 123))
    )
      return el.charCodeAt(0) - 13;
